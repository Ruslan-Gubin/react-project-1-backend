...
class MineGameService {
    ...
    async updateLevelMine(body) {
        ...
        await this.model.findByIdAndUpdate(playerId, { resourceBar: resurceBar }, { returnDocument: 'after' });
        const updateMineFn = async () => {
            try {
                ...
                const playerUpdate = await this.model.findByIdAndUpdate(playerId, { $inc: { population: population }, income: incomeUpdate, resourceBar: resurceBarAfterUpdate, mines: minesUpdate === null || minesUpdate === void 0 ? void 0 : minesUpdate.mines }, { returnDocument: 'after' });
                return playerUpdate;
            }
            catch (error) {
                console.log(error);
