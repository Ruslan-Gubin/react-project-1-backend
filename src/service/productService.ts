...
export class ProductService {
  ...
  async getAllSortProducts(
    req: IRequestQuery<types.GetAllProductModelQuery>,
  ): Promise<{ data: types.IProduct[]; length: number }> {
  ...
  async removeProduct(body: types.RemoveProductModelBody): Promise<types.RemoveProductModelResponse> {
    ...
    await this.model
      .findByIdAndDelete(_id) // remove Product
      .catch((error) => console.log(error));

    await commentService.removeCommentsForTarget(comments).catch((error) => console.log(error)); //remove comments Product
  ...
  async buyProduct(body: types.BuyProductBody): Promise<{ success: boolean }> {
    if (!body) {
      throw new Error('Не получено тело запроса');
    }
